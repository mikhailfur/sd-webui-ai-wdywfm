...
> [!IMPORTANT]
> 首個可執行的 MVP 已提供，包含 Forge Neo 介面、LM Studio/OpenRouter 結構化請求、文字與視覺輸入、驗證，以及僅對提示詞的明確套用。CivitAI 中繼資料強化已實作，進階模型檢索仍屬路線圖工作。
...
| Detailed model cards | `8` | 每次請求的 LoRA 上限 |
| CivitAI timeout | `15 秒` | 中繼資料請求逾時 |
